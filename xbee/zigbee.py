...
BROADCAST_LONG_ADDR = b'\0\0\0\0\0\0\xff\xff'
BROADCAST_SHORT_ADDR = b'\xff\xff'
COORDINATOR_LONG_ADDR = b'\0\0\0\0\0\0\0\0'
COORDINATOR_SHORT_ADDR = b'\0\0'
...
class ZigBee(XBeeBase):
    ...
    def at(self, command, parameter):
        return self.send('at', command=command, parameter=parameter)

    def queued_at(self, command, parameter):
        return self.send('queued_at', command=command, parameter=parameter)

    def remote_at(self, long_addr, short_addr, command, parameter, options=b'\x02'):
        return self.send('remote_at', dest_addr_long=long_addr, dest_addr=short_addr, options=options, command=command, parameter=parameter)

    def tx(self, long_addr, short_addr, data, radius=b'\0', options=b'\0'):
        return self.send('tx', dest_addr_long=long_addr, dest_addr=short_addr, broadcast_radius=radius, options=options, date=data)

    def tx_explicit(self, long_addr, short_addr, src_endp, dest_endp, cluster, profile, data, radius=b'\0', options=b'\0'):
        return self.send('tx_explicit', dest_addr_long=long_addr, dest_addr=short_addr, src_endpoint=src_endp,
                         dest_endpoint=dest_endp, cluster=cluster, profile=profile, broadcast_radius=radius,
                         options=options, date=data)
    ...
    def _parse_IS_at_response(self, packet_info):
        ...
        """
        if packet_info['id'] in ('at_response', 'remote_at_response') \
                and packet_info['command'].lower() == b'is' \
                and packet_info['status'] == b'\x00':
    ...
    def _parse_ND_at_response(self, packet_info):
        ...
        """
        if packet_info['id'] == 'at_response' \
                and packet_info['command'].lower() == b'nd' \
                and packet_info['status'] == b'\x00':

            # Parse each field directly
            result = {
                'source_addr': packet_info['parameter'][0:2],
                'source_addr_long': packet_info['parameter'][2:10]
            }
            ...
            if null_terminator_index + 9 != len(packet_info['parameter']):
                raise ValueError("Improper ND response length: expected {0}, read {1} bytes".format(
                    len(packet_info['parameter']), null_terminator_index + 9))
